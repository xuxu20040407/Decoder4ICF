...
# # ================= 常量定义 =================
# PROGRAM_NAME = 'Multi-1D-Sample-LHS-mjf'
# N_SAMPLES = 6000
...
# LASER_POWER_MIN = [i * 1.3 for i in LASER_POWER]
# LASER_POWER_MAX = [i * 0.7 for i in LASER_POWER]
...
# 固定时间参数（未采样）
LASER_TIME = [0.000000, 0.380000, 0.510000, 0.240000, 0.250000, 
              0.320000, 0.410000, 0.260000, 0.360000, 0.62, 2.21, 0.1300]

# 激光功率参数（需要采样）
LASER_POWER =  [0.000000, 4.490000, 0.000000, 0.00, 3.480000, 
                4.060000, 5.010000, 5.880000, 13.47000, 28.0, 0.000000]
LASER_POWER_MIN = [i * 1.3 for i in LASER_POWER]  # 注意确认缩放方向
LASER_POWER_MAX = [i * 0.7 for i in LASER_POWER]

# ================= 派生参数 =================
n_time_params = len(LASER_TIME)      # 时间参数数量
n_power_params = len(LASER_POWER)    # 功率参数数量
total_dim = n_time_params + n_power_params  # 总参数维度

# ================= 采样函数 =================
def generate_samples(n_samples):
    """生成组合参数（固定时间 + LHS功率）"""
    # 生成LHS功率样本
    power_samples = lhs(n_power_params, samples=n_samples)
    
    # 归一化到功率范围
    for col in range(n_power_params):
        power_samples[:, col] = power_samples[:, col] * (
            LASER_POWER_MAX[col] - LASER_POWER_MIN[col]
        ) + LASER_POWER_MIN[col]
    
    # 创建完整参数矩阵（每个样本前部为固定时间）
    full_params = np.empty((n_samples, total_dim))
    full_params[:, :n_time_params] = LASER_TIME  # 广播时间参数
    full_params[:, n_time_params:] = power_samples
...
def process_task(index, new_dir, laser_params):
    ...
    pm.generate_input_data1D(
        new_dir, 
        index,
        laser_params  # 结构：[时间参数..., 功率参数...]
    )
    pm.run_command_1D(new_dir, index)

def thread_task(index, new_dir, param_grid):
    """线程级任务分发"""
    with ThreadPoolExecutor(max_workers=MAX_WORKERS_THREAD) as executor:
        executor.submit(process_task, index, new_dir, param_grid[index])

# ================= 主函数 =================
def main():
    # 生成组合参数样本
    param_grid = generate_samples(N_SAMPLES)
    Laser_grid=np.zeros((param_grid.shape[0],param_grid.shape[1]+1))
    Laser_grid[:,:-1]=param_grid
    Laser_grid[:,-2]=Laser_grid[:,-3]
    print(Laser_grid.shape)
    print(Laser_grid[0])
    ...
    with ProcessPoolExecutor(max_workers=MAX_WORKERS_PROCESS) as pool:
        list(pool.map(
            partial(thread_task, new_dir=new_dir, param_grid=Laser_grid),
            range(len(Laser_grid))
        ))
