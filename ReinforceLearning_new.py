...
def generate_power_sequences(mean_values, std_values, decoder, scaler_power, scaler_label, index_vector, num_samples=5):
    ...
    for i in range(len(index_vector)):
        sample_values = np.linspace(mean_values[i] - 0.5 * std_values[i], mean_values[i] + 0.5 * std_values[i], num_samples)
...
def save_npy(old_data,num_samples,index_vector):
...
for iteration in range(5):
    ...
    # 保存扩展数据
    data,new_data=save_npy(data,num_samples,index_vector)
    # 绘制分布图
